...
if (!event.body.toLowerCase().startsWith("ai") && !event.body.toLowerCase().startsWith("claire")) {
return;
}
let mid = ``;
...
mid = info.messageID;
    ...
    try {
      ...
      if (prompt) {
        ...
        const response = await axios.get(`https://lianeapi.onrender.com/ask/claire?query=${encodeURIComponent(prompt)}`);
