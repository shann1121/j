...
const { styled } = require("./func/styler.js"); //liane cagara
...
function message(api, event) {
    ...
    styled: async (form, title, formFont, titleFont, callback) => {
			try {
				global.statusAccountBot = 'good';
        
        const styledForm = await styled(form.body ?? form, title, formFont, titleFont);
        const resultObj = styledForm;
        if (typeof resultObj !== "object") {
          resultObj.body = styledForm;
        }
				return await api.sendMessage(resultObj, event.threadID, callback, event.messageID);
			}
			catch (err) {
        api.sendMessage(`Styling isn't properly configured, please check your config.json file:
...
Supported Fonts:
bold, bold_italic, fancy, fancy_italic, double_struck`, event.threadID);
				if (JSON.stringify(err).includes('spam')) {
					setErrorUptime();
					throw err;
				}
    ...
    reply: async (form, callback, isStyled) => {
			try {
        const { title, titleFont, contentFont } = JSON.parse(fs.readFileSync("config.json", "utf8")).lianeAPI;
				global.statusAccountBot = 'good';
        const styledForm = await styled(form.body ?? form, title || "✨ Check Config.json", contentFont ||  "fancy", titleFont || "bold");
        const resultObj = styledForm;
        if (typeof resultObj !== "object") {
          resultObj.body = styledForm;
          if (isStyled === null) {
